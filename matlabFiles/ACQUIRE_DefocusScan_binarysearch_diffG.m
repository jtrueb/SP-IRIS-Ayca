function [handles,output] = ACQUIRE_DefocusScan_binarysearch_diffG(handles,enableFocus,numLoop,z_init,options,scan_width,ROI_flag)
...
tt4 = toc(tt3);
        ...
        if (ind_coarse==1) || (ind_coarse==scan_width)
           ...
           if numLoop < maxLoop
               [handles,output] = ACQUIRE_DefocusScan_binarysearch_diffG(handles,0,numLoop+1,z_init,options,scan_width,ROI_flag);
               return;
           else
               data=[];
               particle_count=-1;
