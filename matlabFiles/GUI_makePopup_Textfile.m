...
function GUI_makePopup_Textfile(textfile,titlename,command)

if exist('command')~=1 % setup message
 if exist('textfile')~=1
   error('Please specify the filename. help GUI_popuptextfile for more info.');
 end
 if exist(textfile)~=2
 ...
 set(f,'resizefcn','GUI_makePopup_Textfile('''','''',''resize_callback'')');
...
set(h,'callback','GUI_makePopup_Textfile('''','''',''okaybutton_callback'')');
