function [handles,output] = ACQUIRE_DefocusScan_v3_RP(handles,enableFocus,numLoop,z_init)
...
%         newOffset = [curROI(3)-newSize(1) 0];
        ...
        for m = 1:sweepSize
            ...
            disp(['ROI - Reference Particles found: ' num2str(particle_count(m)) ', median=' num2str(median(ParticleData.Size(:))) ', stdev= ' num2str(std(ParticleData.Size(:))) ]);
            %            toc(istart);
        end
        ...
        if (numLoop >= maxLoop) || (ind > 1 && ind < sweepSize && value > minimum_particles)
            ...
            %Acquire image
%             [handles,data] = ACQUIRE_scan(handles,'ta ke'); %Acquire a scan
            ...
            elseif ind == sweepSize
                z_pos = z_temp+(sweepSize-4)*sweepStep;
            elseif ind == 1
                z_pos = z_temp-(sweepSize-4)*sweepStep;
            end
            ...
            [handles] = STAGE_MoveAbsolute(handles,zAxes,z_pos);
            numLoop = numLoop+1;
            [handles,output] = ACQUIRE_DefocusScan_v3_RP(handles,0,numLoop,z_init);
        end
