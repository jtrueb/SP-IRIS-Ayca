function [ output ] = filescan_measureCurrentNA( handles,chip_path,chip_name,spot_indices,options)
...
% disp(['Analyzing Image Index: ' num2str(handles.current_index)]);
m=handles.current_index;

 tic;

% parfor m = 1:length(spot_indices)
...
%      clear data;
  ...
  if mirror_flag
      ...
      data_temp(data_temp>bit_depth)=bit_depth;
      data_temp(data_temp<0)=0;
       ...
       pdata{m}.particledata=ZSTACK_generate_particles_measureNA(data_temp,handles);
...
disp(['Analysis time: ' num2str(toc)]);
 output.pdata=pdata;
