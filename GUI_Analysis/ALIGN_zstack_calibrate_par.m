...
neg_thresh_min=.99;
pos_thresh_min=1.01;
large_thresh_min=1.02;
large_thresh_max=1.2;
...
parfor m=1:length(analysis_range)
    disp(['Processing slice: ' num2str(m)]);
    tic;
    im_temp=zStack.slice(analysis_range(m)).image;
    
    if mirror_flag
        im_temp=im_temp.*median(mir_temp(:))./mir_temp;
    end
    
    im_crop=im_temp(im_crop_indices(1,:),im_crop_indices(2,:)); 
...
%         neg_thresh_min=.99;
%         pos_thres_min=1.01;
%         large_thresh_min=1.05;
%         large_thresh_max=1.2;

    pos_mask_morph=(particledata_quick.Contrasts>pos_thresh_min)&(particledata_quick.Contrasts<large_thresh_min);
    pos_mask_large = (particledata_quick.Contrasts>=large_thresh_min)&(particledata_quick.Contrasts<large_thresh_max);
    neg_mask=particledata_quick.Contrasts<neg_thresh_min;
...
im_crop=zStack.slice(focus_slice).image(im_crop_indices(1,:),im_crop_indices(2,:)); %in focus slice for saturated region detection
temp_xpix=tempvars(1).xpix;
temp_ypix=tempvars(1).ypix;
for i=1:length(temp_ypix)
    for j=1:length(temp_xpix)
        if im_crop(round(temp_ypix(i)),round(temp_xpix(j)))>=handles.fullwell;
            density_map(i,j)=0;
...
z_max=(zStack.slice(end).z_pos);
z_min=zStack.slice(1).z_pos;

% density_mask=density_map>(max(density_map(:))*density_cutoff);
density_mask=(focus_map<z_max)&(focus_map>z_min);
...
output.y_vector=tempvars(2).ypix;
 ...
 x_allpix=tempvars(1).x_allpix;
  y_allpix=tempvars(1).y_allpix;

output.x_full=x_allpix;
output.y_full=y_allpix;

screw_span=1.25; %inches
...
alignment.x_screw_turns=alignment.plane_xslope*screw_span*tpi; %in turns
...
%     alpha(.3);
    ...
    temp_image=zStack.slice(analysis_range(focus_slice)).image(im_crop_indices(1,:),im_crop_indices(2,:));
    temp_min=median(temp_image(:))-2*std(temp_image(:));
    temp_max=median(temp_image(:))+2*std(temp_image(:));
    
    imagesc(temp_image,[temp_min,temp_max]);
