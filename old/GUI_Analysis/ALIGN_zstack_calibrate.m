function [ output ] = ALIGN_zstack_calibrate(handles,fullpath)
...
%             exposure_calibrated=exposure_current*(.65/intPeak_detected);
    ...
    if m==1
        xpix=kde.x(1,kde_indices);
        x_allpix=kde.x(kde_indices,kde_indices);
        y_allpix=kde.y(kde_indices,kde_indices);
        ypix=kde.y(kde_indices,1);
...
%     pdf_pdata(:,:,1,m)=kde_pdata_focus(m).pdf.*kde_pdata_focus(m).npart;
...
output.x_vector=xpix;
output.y_vector=ypix;

output.x_full=kde.x(kde_indices,kde_indices);
output.y_full=kde.y(kde_indices,kde_indices);
