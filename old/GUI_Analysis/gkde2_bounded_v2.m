function p=gkde2_bounded_v2(x,limits,grid_dim,h_input)
...
%     x2=linspace(xmin(2),xmax(2),N);
    x1=linspace(xylim(1),xylim(3),N);
    x2=linspace(xylim(2),xylim(4),N);
...
% calculate icdf
if isfield(p,'alpha')
    p.C=contour(p.x,p.y,p.cdf,[p.alpha p.alpha]);
    p.icdfx = min(p.C(1,2:end));
    p.icdfy = min(p.C(2,2:end));
end
